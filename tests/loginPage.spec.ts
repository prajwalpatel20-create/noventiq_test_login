...
test.describe('Login Page', () => {
    ...
    test('tc - 01: Verify error message when both username and password are empty @negative', async ({ pageObjectManager }) => {
    ...
    test('tc - 02: Verify error message when username is invalid and password is empty @negative', async ({ pageObjectManager }) => {
        await pageObjectManager.loginPage.login(wrongUsername, emptyPassword);
        const errorMessage = await pageObjectManager.loginPage.getErrorMessage();
        expect(errorMessage).toBe(invalidUsernameMsg);
    });

    test('tc - 03: Verify error message when username is correct but password is empty @negative', async ({ pageObjectManager }) => {
        await pageObjectManager.loginPage.login(correctUsername, emptyPassword);
        const errorMessage = await pageObjectManager.loginPage.getErrorMessage();
        expect(errorMessage).toBe(invalidPasswordMsg);
    });

    test('tc - 04: Verify error message when username is empty and password is correct @negative', async ({ pageObjectManager }) => {
        await pageObjectManager.loginPage.login(emptyUsername, correctPassword);
        const errorMessage = await pageObjectManager.loginPage.getErrorMessage();
        expect(errorMessage).toBe(invalidUsernameMsg);
    });

    test('tc - 05: Verify error message when username is correct but password is incorrect @negative', async ({ pageObjectManager }) => {
        await pageObjectManager.loginPage.login(correctUsername, wrongPassword);
        const errorMessage = await pageObjectManager.loginPage.getErrorMessage();
        expect(errorMessage).toBe(invalidPasswordMsg);
    });

    test('tc - 06: Verify user can successfully login using correct username and correct password @positive', async ({ pageObjectManager }) => {
    ...
    test('tc - 07: Verify that the application navigates to the correct URL after successful login @positive', async ({ pageObjectManager }) => {
        await pageObjectManager.loginPage.login(correctUsername, correctPassword);
        const currentUrl = await pageObjectManager.homePage.getCurrentUrl();
        expect(currentUrl).toContain(url);
    });

    test('tc - 08: Verify that the correct page title is displayed after successful login @positive', async ({ pageObjectManager }) => {
        await pageObjectManager.loginPage.login(correctUsername, correctPassword);
        const title = await pageObjectManager.homePage.getPageTitle();
        expect(title).toBe(pageTitle);
    });

    test('tc - 09: Verify that the correct success message is displayed after successful login @positive', async ({ pageObjectManager }) => {
        await pageObjectManager.loginPage.login(correctUsername, correctPassword);
    ...
    test('tc - 10: Verify that the logout button is visible and enabled after successful login @positive', async ({ pageObjectManager }) => {
        await pageObjectManager.loginPage.login(correctUsername, correctPassword);
